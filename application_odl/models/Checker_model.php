...
class Checker_model extends CI_Model
{
    ...
    {
        $this->db->select("ch.*, c.*, s.*,b.big_grouping_cust,
    CASE 
    ...
    END AS is_banking", false); // ← ini penting, escape = false
        ...
        $this->db->group_by("ch.id_checker");
    ...
    public function refresh_mv_checker_summary()
    {
        ...
        $sql = "
            INSERT INTO mv_checker_summary (
                id_courier, id_checker, runsheet_date, create_date, upload_by,
                zone, status_checker, qty_awb, qty_sesuai, qty_revisi, qty_tidak_sesuai,
                courier_name, zone_name, origin_code, role, name, username, no_runsheet,
                success_pod, in_progress_pod, amount, cod_undelivered, status_pod,leaderboard_year,leaderboard_month
            )
            SELECT
                ch.id_courier,
                ...
                YEAR(runsheet_date) AS leaderboard_year,
    MONTH(runsheet_date) AS leaderboard_month
            FROM checker ch

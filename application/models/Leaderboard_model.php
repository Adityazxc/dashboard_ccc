...
$this->db->select('
    ...
    c.courier_name,
    z.zone,
    SUM(l.kpi) AS total_kpi,
    SUM(l.hrs) AS total_hrs,
    SUM(l.total_poin) AS total_poin,
    SUM(l.succes_rate) AS total_success_rate,
    SUM(l.minus_poin) AS total_minus_poin,
    ...
    MAX(m.runsheet_date) AS last_runsheet 
');
$this->db->from('mv_leaderboard_summary l');
$this->db->join('courier c', 'c.id_courier = l.id_courier', 'left');
$this->db->join('mv_checker_summary m', 'm.id_courier = l.id_courier');
...
function count_all_courier()
{
    ...
    public function _get_data_awb($id_courier)
    {
        ...
        $this->db->select('id_courier, SUM(qty_awb) AS sum_qty_awb');
        $this->db->from('mv_checker_summary');
        $this->db->where('id_courier', $id_courier);

        // Tambahkan filter tanggal dinamis
        $this->db->where('Date(runsheet_date) >=', $dateFrom);
        $this->db->where('Date(runsheet_date) <=', $dateThru);

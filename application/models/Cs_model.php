...
class Cs_model extends CI_Model
{
    ...
    private function _getdatatables_report()
    {
              
        $this->db->select('c.*, u.account_name');
        $this->db->from('customers c');
        $this->db->join('users u', 'c.id_send_email = u.id_user');
        $this->db->order_by('c.date_send_email', 'desc');
        $this->db->where('c.id_send_email IS NOT NULL');
    ...
    function getdatatables_report()
    {
        $this->_getdatatables_report();
        if (@$_POST['length'] != -1)
            $this->db->limit(@$_POST['length'], @$_POST['start']);
        $query = $this->db->get();
        return $query->result();
    ...
    function count_filtered_marketing()
    { 
        $this->_getdatatables_marketing();
        $query = $this->db->get();
        return $query->num_rows();

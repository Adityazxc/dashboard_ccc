...
<form id="filterForm">
    <div class="row">
        ...
        <!-- ORIGIN -->
        <div class="form-group col-md-2">
            <label for="origin" class="form-label required">Origin Destination:</label>
            <select class="form-select select2" name="origin" id="origin" <?= (!$full_access) ? 'disabled' : '' ?>>
                ...
                <?php foreach ($get_origins_array as $get_origin): ?>
                    <option value="<?= $get_origin['tariff_code'] ?>" <?= (!$full_access && $get_origin['tariff_code'] == $origin) ? 'selected' : '' ?>>
                        <?= $get_origin['city_name'] ?>- <?= $get_origin['district_name'] ?> (<?= $get_origin['tariff_code'] ?>)
                    </option>
                <?php endforeach; ?>
            </select>
        </div>

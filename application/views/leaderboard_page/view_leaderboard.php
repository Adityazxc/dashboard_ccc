...
<form id="filterForm">
    <div class="row">
        ...
        <!-- <div class="form-group col-md-3">
            ...
            <select class="form-select select2" name="origin" id="origin" <?= (!$full_access) ? 'disabled' : '' ?>>
                ...
                <?php foreach ($get_origins_array as $get_origin): ?>
                    <option value="<?= $get_origin['origin_code'] ?>" 
                        <?= ($get_origin['origin_code'] == $selected_origin) ? 'selected' : '' ?>>
        ...
        <input type="hidden" name="origin" id="origin" value="<?= $selected_origin ?>">
...
<div class="bg_table bg-white w-100 h-100">

    <!-- Top Kurir -->
    <div class="row ">
        <div class="table-responsive container table-scroll">
...
<style>
    .table-scroll {
        ...
        /* opsional kalau mau ada garis luar */
        border-radius: 10px;
        /* opsional */
        background: white;
        /* biar kelihatan rapi */
...
<script>
    $(document).ready(function () {
        ...
        let $box = $('.table-scroll');
        let speed = 50; // semakin kecil -> semakin cepat

        function scrollDown() {
            $box.animate(
                { scrollTop: $box[0].scrollHeight },
                $box[0].scrollHeight * speed,
                'linear',
                scrollUp
            );
        }
        ...
        // mulai langsung dari atas → bawah → atas → bawah (loop)
        scrollDown();
    });

    ...
    <div class="col-md-4">
        <div class="card">
            <div class="card-header">
                <div class="card-title">Category Shipment</div>
            </div>
            <div class="card-body">
                <div class="chart-container" style="position: relative; height:350px;">
                    <canvas id="deliveryComparisonChart"></canvas>

                    <div id="deliveryEmptyState" style="display:none;
                        position:absolute;
                        top:50%;
                        left:50%;
...
<script>
    ...
    var jumlah = () => {
        ...
        $.ajax({
            url: "<?= base_url('last_mile/summary_dashboard') ?>",
            dataType: "JSON",
            type: "POST",
            data: formData,
            success: (r) => {
...
<script>
    ...
    function loadServiceChart() {
        ...
        $.ajax({
            url: "<?= base_url('last_mile/getServiceChart') ?>",
            type: "POST",
            data: formData,
            dataType: "JSON",
    ...
    function loadPayTypeChart() {
        ...
        $.ajax({
            url: "<?= base_url('last_mile/getPayTypeChart') ?>",
            type: "POST",
            data: formData,
            dataType: "JSON",
            success: function (response) {
                ...
                if (response.success) {
                    ...
                    window.PayTypeChartInstance = new Chart(ctx, {
                        ...
                        data: {
                            datasets: [{
                                data: PayTypeCounts,
                                backgroundColor: generateChartColors(PayTypeCounts.length),
                                borderWidth: 0
                            }],
    ...
    function loadDeliveryComparisonChart() {

        var formData = $('#filterForm').serialize();

        $.ajax({
            url: "<?= base_url('last_mile/getDeliveryComparisonChart') ?>",
            type: "POST",
            data: formData,
            dataType: "JSON",
            success: function (response) {

                var canvas = document.getElementById('deliveryComparisonChart');
                var ctx = canvas.getContext('2d');
                ...
                if (response.success) {
                    ...
                    var onProses = response.onProses || [];

                    var total = delivered.reduce((a, b) => a + b, 0) +
                        onProses.reduce((a, b) => a + b, 0);

                    if (total === 0) {
                        $('#deliveryEmptyState').show();
                        return;
                    }

                    $('#deliveryEmptyState').hide();
    ...
    function loadTopCustomers() {
        ...
        $.ajax({
            url: "<?= base_url('last_mile/getTopCustomers') ?>",
            type: "POST",
            data: formData,
            dataType: "JSON",
            success: function (response) {
